...
const AllFeed = ({ id, qpost, setqpost, cudetails, groupId }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToPosts((posts) => {
      ...
      setqpost(filteredPosts) // Also update parent state for compatibility
      setLoading(false)
    }, {})
  ...
  return (
    <div>
      ...
      {allPosts && allPosts.length > 0 ? (
        allPosts.map(post => {
          // Render quiz posts with QuizPostCard
          if (post.type === 'quiz') {
            return (
              <QuizPostCard 
                key={post._id || post.postId} 
                post={post}
                onQuizCreated={handlePostCreated}
              />
            )
          }
          // Render questions and regular posts with Post component
          return (
            <Post 
              key={post._id || post.postId} 
              post={post} 
